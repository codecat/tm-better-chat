class ChatWindow : IChatMessageReceiver
{
	...
	//TODO: Remove these debug options, it doesn't make sense in this class
	void AddDebugLine(bool json)
	{
		auto emoteKeys = Emotes::g_emotes.GetKeys();

		string login = "c5jutptORLinoaIUmVWscA";
		string author = DummyData::PlayerName();
		string text = "";
		...
		for (uint j = 0; j < parse.Length; j++) {
			...
			text += parse[j];
			if (Math::Rand(0, 10) == 0) {
				text += " " + emoteKeys[Math::Rand(0, emoteKeys.Length)];
			}
			if (Math::Rand(0, 20) == 0) {
				text += " miss";
			}
		...
		if (json) {
			...
			js["nickname"] = author;
			js["text"] = text;
			AddLine("CHAT_JSON:" + Json::Write(js));
		} else {
			AddLine("[$<" + author + "$>] " + text);
		}
	}
	...
	void RenderMenu()
	{
		if (UI::BeginMenu("Better Chat Debug")) {
			...
			if (UI::MenuItem("Add a Json line of nonsense")) {
				AddDebugLine(true);
			}
			UI::EndMenu();
		}
